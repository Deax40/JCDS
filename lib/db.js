...
export function getPool() {
  if (!pool) {
    ...
    // Configuration SSL pour Supabase et autres bases de données
    ...
    // Accepte les certificats auto-signés en développement et production
    const sslConfig = process.env.DATABASE_URL.includes('supabase.co') ||
                      process.env.DATABASE_URL.includes('https') ||
                      process.env.DATABASE_URL.includes('ssl=true')
      ? {
          rejectUnauthorized: false,
          // Options supplémentaires pour éviter les erreurs SSL et certificats auto-signés
