...
export function getPool() {
  if (!pool) {
    ...
    // Configuration SSL pour Supabase et autres services PostgreSQL hébergés
    // IMPORTANT: rejectUnauthorized: false est nécessaire pour Supabase et Vercel
    const isProduction = process.env.NODE_ENV === 'production';
    const isVercel = process.env.VERCEL === '1';

    // Déterminer la configuration SSL
    let sslConfig;
    if (isProduction || isVercel || process.env.DATABASE_URL.includes('supabase.co')) {
      // En production ou sur Vercel, accepter les certificats auto-signés
      sslConfig = {
        rejectUnauthorized: false
      };
    } else {
      // En développement local, pas de SSL (sauf si explicitement demandé)
      sslConfig = process.env.DATABASE_URL.includes('sslmode=require')
        ? { rejectUnauthorized: false }
        : false;
    }
