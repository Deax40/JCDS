...
export function getPool() {
  if (!pool) {
    ...
    // Sur Vercel (production), on doit souvent accepter les certificats auto-signés
    // pour les bases de données gérées (Supabase, Neon, etc.)
    const isProduction = process.env.NODE_ENV === 'production';

    // On active SSL avec rejectUnauthorized: false si on est en prod OU si l'URL contient supabase
    // Cela couvre le cas où le mot clé 'supabase' n'est pas dans l'URL mais qu'on est hébergé
    const sslConfig = (isProduction || process.env.DATABASE_URL.includes('supabase'))
